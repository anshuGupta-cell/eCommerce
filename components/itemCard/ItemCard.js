    ...
    <div className="grid p-2 bg-blue-50 dark:bg-blue-50/10 gap-3 rounded-lg ">
      <div >
        <ul className="grid place-items-center">
          <img className="h-full w-full object-cover rounded-lg aspect-square" src={item.item_pic} alt="" />
        </ul>
        <ul className="p-1">
          <h3 className="font-bold">{item.item_name}</h3>
          <p className="text-sm">{item.description}</p>
          <p className="text-sm font-medium">Price: {item.price}</p>
          {qty?<p className="text-sm">Quantity: {item.qty}</p>:<p className="text-sm">Stock: {item.stock}</p>}
        </ul>
      </div>
      {removeItem && <Button onClick={() => removeItem(item.item_id)}>Remove</Button>}
      {changeQuantity && <ul
