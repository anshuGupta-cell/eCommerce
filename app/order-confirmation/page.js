...
const About = () => {
    ...
    const orderConfirm = async (oid) => {
        try {
            ...
            const data = await res.json()
            fetchData()
            toast(`Order ${oid} is confirmed`)
        } catch (error) {
            toast("Failed to update order status to confirmed")
        }
    }

    const orderCancel = async (oid, e) => {
            e.preventDefault()
            try {
                const res = await fetch(`/api/order-status`, {
                    method: "PATCH",
    ...
    return (
        <section className="py-40 bg-gradient-to-r from-indigo-50 via-purple-50 to-pink-50  dark:bg-gradient-to-r dark:from-slate-900 dark:to-slate-800 text-black dark:text-white/80 px-2">
            <div className="max-w-5xl m-auto grid res-grid-280 gap-2 ">
                ...
                {orders.map((order) => (
                    order.status !== "confirmed" && order.status !== "cancelled" ? <div key={order.oid} className="grid gap-2 bg-slate-800 rounded-lg p-2 ">
                        <ul className="">
                        ...
                        <ul className="flex w-full justify-around">
                            <Modal
                                ...
                                isActive={true}
                            >
                                <form className="grid max-w-3xl m-auto bg-slate-800 p-3 gap-5 z-10" onSubmit={(e) =>orderCancel(order.oid, e)}>
                            ...
                            <button onClick={()=>orderConfirm(order.oid)} className="bg-green-500 rounded-lg py-1 px-3">Confirm</button>
                        </ul>
